...
{
    std::filesystem::path outputPath = std::filesystem::current_path() / "Orders.txt";
    std::filesystem::create_directories(outputPath.parent_path());

    file.open(outputPath.string());

    if (!file.is_open()) {
        std::cerr << "Error opening file for writing: " << outputPath.string() << std::endl;
        return;
    }
    ...
    file.close();
    std::cout << "Orders written to " << outputPath.string() << " successfully!" << std::endl;
}

void GenerateOrders::createInitialOrders(int numberOfOrders, int centreOfBook)
{
    std::filesystem::path outputPath = std::filesystem::current_path() / "Generate_Orders" / "initialOrders.txt";
    std::filesystem::create_directories(outputPath.parent_path());
